...
const HoverCard: React.FC<HovercardProps> = ({idx, nodeId, width, handleBottomClick, handleLeftClick, handleRightClick, handleTopClick, handleCenterClick}) => {
  ...
  return (
    <div
      key={idx}
      className={`card h-full shadow-sm border border-indigo-600`}
      style={{width}}
    >
    <div>
        ...
        <div className={'h-36 w-full flex'}>
            ...
            <div className={'h-full w-full'}  onClick={handleCenterClick}>
                <div>
                    {nodeId}
                </div>
            </div>
